...
namespace CS170
{
    ...
    public:
		...
		// Inserts a new node into a sorted list. The node will be inserted 
		// at a specific location based on its numeric value.
		void insert_node_sorted(int value_);

		// Merges the contents of another sorted list into this list (also sorted).
		// The resulting list will contain all elements from both lists in the proper order.
		void merge(List &list_);

		// Removes all duplicate elements from a sorted list and
